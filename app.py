...
def get_stock_data(ticker, period='1y'):
    try:
        ...
        short_trend = "Bullish" if (
            latest['EMA10'] > latest['EMA20'] and 
            latest['MACD_short_diff'] > 0 and
            latest['RSI14'] > prev['RSI14']
        ) else "Bearish"
        
        # Medium: SMA20/SMA50 + MACD + RSI
        med_trend = "Bullish" if (
            latest['SMA20'] > latest['SMA50'] and 
            latest['MACD_med_diff'] > 0 and
            latest['RSI14'] > 40
        ) else "Bearish"
        
        # Long: SMA50/SMA200 + MACD
        long_trend = "Bullish" if (
            latest['SMA50'] > latest['SMA200'] and 
            latest['MACD_long_diff'] > 0
        ) else "Bearish"
        ...
        return {
            ...
            'macd_short_signal': float(latest['MACD_short_signal']) if not pd.isna(latest['MACD_short_signal']) else 0,
            'macd_short_diff': float(latest['MACD_short_diff']) if not pd.isna(latest['MACD_short_diff']) else 0,
            'macd_med': float(latest['MACD_med']) if not pd.isna(latest['MACD_med']) else 0,
            'macd_med_signal': float(latest['MACD_med_signal']) if not pd.isna(latest['MACD_med_signal']) else 0,
            'macd_med_diff': float(latest['MACD_med_diff']) if not pd.isna(latest['MACD_med_diff']) else 0,
            'macd_long': float(latest['MACD_long']) if not pd.isna(latest['MACD_long']) else 0,
            'macd_long_signal': float(latest['MACD_long_signal']) if not pd.isna(latest['MACD_long_signal']) else 0,
            'macd_long_diff': float(latest['MACD_long_diff']) if not pd.isna(latest['MACD_long_diff']) else 0,
...
def get_holdings():
    ...
    for holding in holdings:
        ...
        if stock_data:
            ...
            holding_dict['week52_range'] = f"${stock_data['week52_low']:.2f} - ${stock_data['week52_high']:.2f}"
            ...
            # P/L in percent
            holding_dict['pl_percent'] = ((holding_dict['value'] - entry_value) / entry_value) * 100 if entry_value > 0 else 0
            
            # Price change percent
            pct_change = ((stock_data['current_price'] - holding_dict['entry_price']) / holding_dict['entry_price']) * 100
            ...
            if 'Earnings Date' in calendar.index:
                ...
                if pd.notna(earnings_date):
                    ...
                    if isinstance(earnings_date, pd.Timestamp):
                        return jsonify({
                            'date': int(earnings_date.timestamp()),
                            'available': True
                        })
...
def get_wishlist():
    ...
    for item in wishlist:
        ...
        if stock_data:
            ...
            item_dict['week52_range'] = f"${stock_data['week52_low']:.2f} - ${stock_data['week52_high']:.2f}"
...
def add_to_wishlist():
    try:
        ...
        if not validate_ticker(ticker):
            return jsonify({'error': f'Invalid ticker symbol: {ticker}. Please check the symbol and try again.'}), 400
...
if __name__ == '__main__':
    # Run on all interfaces so phone can access it
